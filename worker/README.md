...
The middleware. `POST /run?usecase=founders-copilot|on-it` → `runStages` (plan→tool→render) → SSE
...
<https://ldnmxx-hack-worker.cloudflare-driveway392.workers.dev>. Files: `src/worker.ts` (`runStages`),
