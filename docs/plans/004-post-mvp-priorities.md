...
**Originally handoff 004 — "start here: post-MVP priorities (doc-truth → usecases/*.json → flagship
depth)" (updated 2026-07-04).**
...
(now the "Handoff 003" section of `002-phase1-first-e2e.md`). Job: pick up this plan (the ticket).

### Read first (don't re-gather context)
1. **This plan** — the plan **+ a full code/file/source map + env gotchas + the doc-truth audit**.
   Everything you need to act without re-mapping the repo.
2. The "Handoff 003" section of `002-phase1-first-e2e.md` — what Phase 1 built (now partly superseded;
   note its "no model call" line is stale). `docs/usecase-workflows.md` — the `usecases/*.json` stage-def
   schema for Tier 1.
...
- The "Handoff 003" section of `002-phase1-first-e2e.md` — its "stub makes no model call" line is now
  **false**.
