...
**Originally handoff 008 — "PR-3: HUD status bar (Demo⇄Live + mode/model + cost) — approved, build it"
(updated 2026-07-10).**

**Status:** **built + merged (PR-3).** This is **plan 007's PR-3**, reshaped from "a cost chip" into a HUD
status bar + Demo/Live toggle. The living phase-2 resume doc is `007-phase2-model-pipeline.md` (its
Progress table is ticked for this merge); **this plan is the detailed PR-3 plan** with a full source
map — trust it, don't re-explore.
...
`tsc --noEmit` green → UI. Full file:line source map is below in this plan.

### Recurring checklist (pre-answered below)
CHANGELOG · README `Switches` (fix `?demo=1` — now a UI toggle) · `architecture.md` (USAGE event) ·
`UserStory.md` (mode-transparency story) · no new env/url/cli · open a "Demo/Live toggle" issue (or note in #18) plus
