...
**Originally handoff 007 — "Phase 2 model-driven pipeline (#18) + usecase expansion — approved, build it"
(updated 2026-07-09).**

**Status:** **PR-1/2/3 shipped; PR-4 (Arize live) next — blocked account-side #50; PR-5 superseded by the
civic pivot (plans 010–012).** Originally scoped as **5 phased, strict-TDD PRs** (branch per PR →
CI-gated → squash-on-green → prune). The **Source map** at the bottom of this plan is the current code
state — trust it instead of re-exploring.
...
Build the rest as phased, strict-TDD PRs. This plan carries a full **Source map** (exact files,
signatures, line numbers) — trust it; do **not** re-explore the codebase.

### Progress — this table is the authoritative PR state (update after each PR merges)
...
| PR-3 | **HUD status bar** — Demo⇄Live toggle + mode/model + cost chip (`USAGE` event). *Reshaped from "cost chip"; detailed plan: **`008-hud-status-bar.md`*** | [x] merged |
...
and the HUD now emits a terminal `USAGE` event → an honest Demo⇄Live chip (plan `008-hud-status-bar.md`).
→ start **PR-4** = Arize live resolution + notes (#50 / `AGENT_LEARNINGS.md`), which **closes #18**.
...
`make test` + `tsc --noEmit` green → PR. See this plan's PR-3 section for the source map.
...
- **Docs:** `CHANGELOG`; `008-hud-status-bar.md` resume point (final step of the plan).
