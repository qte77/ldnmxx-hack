...
**Status: P0 in progress.** Row 1 (tokens & fonts) is being executed now. Rows 2–11 not started.

**What's next, in order:**
1. **P0 (rows 1–3)** — foundation: tokens/fonts (row 1, in progress), shared contracts (row 2:
   `prefs.ts`, per-submission routing, shell split), catalog fields (row 3). Rows 1–3 are actually
   file-disjoint (no overlapping files) and COULD run in parallel worktrees — the plan originally
   called P0 "serial" out of caution; relax that if useful, but everything in rows 4–7 still needs all
   of rows 1–3 merged first.
2. **P1–P4 (rows 4–7, parallel, one worktree + branch each)** — Home screen, Result sheet, Settings
   screen, Docs. Dispatch via `Agent(isolation: "worktree")` once rows 1–3 are merged; cap at 3
   concurrent background worktree agents.
3. **P5 (rows 8–10, serial, after rows 4–7 merged)** — e2e/CI updates, version bump to v2.0.0, deploy +
   live verification.
...
2. Launch up to 3 `Agent(subagent_type: "general-purpose", isolation: "worktree")` calls in ONE
   message (rows 4–6) — each a fresh worktree off latest `main`, `npm ci` before any edit, own PR.
3. Row 7 (Docs) only needs row 1, so it can run as a 4th slot alongside or immediately after.
4. Brief every worktree agent with its row, the "Token strategy"/"Source map" sections (don't
   re-derive), and the Bash deny-list + 404.html watch-outs.
5. Expected conflicts: near-zero except `ui/eslint.config.js`/`tokens.css`, which belong to row 1 only
   — rows 4–7 consume, never modify them.
