...
pass) is in this PR. Rows 9–10 remain; row 10 is owner-gated (no `CLOUDFLARE_API_TOKEN` here, and per
...
| 8 | E2E + CI + bundle ceiling | in this PR (found + fixed a real sheet-close bug, see the plan's Handoff note) |
| 9 | Version + release (v2.0.0) | not started |
| 10 | Deploy + live verification | not started (owner-gated) |
| 11 | Handoffs README housekeeping | done earlier this arc (see `docs/handoffs/README.md`); this progress table + the plan's remaining-work table are the rest of row 11's job, done in this PR |
